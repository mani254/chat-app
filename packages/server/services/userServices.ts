import { User, UserQueryParams, UsersFetchResult } from '@workspace/database';
...
class UserService {
  private getMatchState(query: UserQueryParams): Record<string, any> {
    ...
    if (isOnline !== undefined) {
      const normalizedIsOnline = typeof isOnline === 'string' ? isOnline === 'true' : Boolean(isOnline);
  ...
  async fetchUsers(query: UserQueryParams): Promise<UsersFetchResult> {
    try {
      const { sortBy = 'createdAt', orderBy = 'desc', page, limit, fetchFields } = query;
      ...
      const userStages: any[] = [{ $sort: { [sortBy]: orderBy === 'desc' ? -1 : 1 } }, { $skip: skip }];
