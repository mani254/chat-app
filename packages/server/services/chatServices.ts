...
import { Chat, ChatDocument, ChatQueryParams, CreateChatPayload, PopulatedChatDocument } from '@workspace/database';
...
class ChatService {
  private getMatchStage(query: ChatQueryParams): Record<string, any> {
    ...
    if (isGroupChat !== undefined) {
      const normalizedIsGroupChat = typeof isGroupChat === 'string' ? isGroupChat === 'true' : Boolean(isGroupChat);
  ...
  async fetchChats(query: ChatQueryParams): Promise<{ totalItems: number; chats: PopulatedChatDocument[] }> {
    try {
      const { sortBy = 'updatedAt', orderBy = 'desc', page, limit, fetchFields } = query;

      const skip = query?.skip || (page && limit) ? (Number(page) - 1) * Number(limit) : 0;
      const matchStage = this.getMatchStage(query);

      const chatStages: any[] = [{ $sort: { [sortBy]: orderBy === 'desc' ? -1 : 1 } }, { $skip: skip }];
      ...
      const pipeline = [
        ...
        {
          $project: {
            ...(fetchFields && Object.keys(fetchFields).length > 0 ? fetchFields : {}),
            users: {
              $map: {
                input: '$users',
                as: 'user',
  ...
  async createChat(data: CreateChatPayload, userId: Types.ObjectId): Promise<ChatDocument> {
    ...
    if (!isGroupChat) {
      const userNames = (populatedChat.users as any[]).map((u) => u.name.trim()).sort((a, b) => a.localeCompare(b));
