...
function generateAccessibleColor(): string {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    const l = 20 + Math.floor(Math.random() * 18); // 20–38%
...
userSchema.pre('save', async function () {
  // Assign accessible color if missing
  if (
    !this.color ||
 ...
 * Never exported. Used only within the DAL to wire schema → entity.
 */
 ...
 *
 * This is intentionally NOT exported. Only UserRepository may use it.
 * Consumers must go through the repository — never directly through the model.
 */
export const UserModel = model<RawUserDocument>(
  'User',
  userSchema,
