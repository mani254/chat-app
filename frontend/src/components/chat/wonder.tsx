  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button
          aria-label="New Chat"
          className="flex items-center justify-center w-10 h-10 rounded-full bg-background shadow-md border border-background-accent hover:bg-background transition cursor-pointer text-background hover:text-background-accent"
        >
          <Plus className="w-5 h-5 text-primary" />
        </button>
      </DialogTrigger>

      <DialogContent className="max-w-md w-full rounded-2xl p-0 border  shadow-xl bg-background-fix">
            ...
            {loadingUsers && users.length === 0 ? (
              ...
              users.map((user) => (
                ...
                >
                  ...
                  <div className="flex flex-col bg-background-fix">
