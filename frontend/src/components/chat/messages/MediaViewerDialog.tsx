...
export default function MediaViewerDialog({ open, onOpenChange, items, currentIndex, onNavigate }: MediaViewerDialogProps) {
  ...
  const isDoc = isDocument(item.type, item.url);
  const fileCategory = getFileCategory(item.type, item.name);
  ...
  console.log('Document Debug:', {
    ...
    name: item.name,
    isDoc,
    previewUrl: isDoc ? getDocumentPreviewUrl(item.url, item.type, item.name) : null,
    fileName: item.url ? item.url.split('/').pop() : item.name
  });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-5xl p-0 overflow-hidden min-h-[520px] bg-background" showCloseButton onClick={(e) => e.stopPropagation()}>
