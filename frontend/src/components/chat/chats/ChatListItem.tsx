...
const ChatListItem = ({ chat, currentUserId, isChatActive, haveUnreadMessages }: ChatListItemProps) => {
  ...
  return (
    <button
      ...
      className={cn(
        "relative w-full flex items-center gap-3 px-3 py-2.5 rounded-xl transition text-left cursor-pointer border-b border-border/60",
        "hover:bg-background-accent/60",
        isChatActive && "bg-primary/10 border border-l-4 border-primary/40 font-semibold hover:bg-primary/10",
      )}
    >
      ...
      {haveUnreadMessages && (
        <div className="bg-primary w-2 h-2 rounded-full absolute bottom-3 right-3 ring-2 ring-background" />
      )}
      {
